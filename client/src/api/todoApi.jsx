...
// axios와 jwtAxios를 사용한 API 요청 함수들을 정의한 유틸 모듈
...
export const postAdd = async (todo) => {
  const res = await jwtAxios.post(`${prefix}/`, todo); // ← jwtAxios로 TodoController 호출
  return res.data;
};
...
export const putOne = async (todo) => {
  const res = await jwtAxios.put(`${prefix}/${todo.tno}`, todo); // ← jwtAxios로 TodoController 호출
  return res.data;
};
...
export const deleteOne = async (tno) => {
  const res = await jwtAxios.delete(`${prefix}/${tno}`); // ← jwtAxios로 TodoController 호출
  return res.data;
};
