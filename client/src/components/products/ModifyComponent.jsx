...
import ResultModal from "../common/ResultModal";        // 임포트 시 훅이나 컴포넌틀르 바로 못찾을 때가 있다..
...
const ModifyComponent = ({pno}) => {
    ...
    */
    ...
    useEffect(() => {
        ...
        setFetching(true)   // FatchingModal 보이기
        
        getOne(pno).then(data => {
            setProduct(data)    
            setFetching(false)  // FatchingModal 숨김
        })
    },[pno])
    ...
    const handleClickModify = () => {   // 수정 시 새로운 이미지 파일을 추가하는 부분은 기존의 신규 상품 등록과 동일하다. 그래서 코드도 같다.
        ...
        formData.append("delFlag", product.delFlag)//"사용(flase)" 이면 실제 삭제가 아닌 값만 바꾼다. "삭제(true)" 이면 실제 삭제가 이루어진다.
    ...
    const closeModal = () => {
        ...
        setResult(null) // 결과 모달창을 다시 안보이게..
    }
    ...
    return (
        <div className="p-4 w-full bg-white">
            <div className="border-2 border-sky-200 mt-10 m-2 p-4">
                ...
                {fetching ? <FetchingModal/> : <></>}   {/* fetching 의  true/false 에 의해 FetchingModal 을 보여줄지 말지 결정 */}
